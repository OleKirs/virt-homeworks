...
**Вариант выбора: KVM или Xen в редакции Open Source.**  
...
**Вариант выбора: Hyper-V Server 2019**  
...
**Альтернатива: кластер из Windows Server с ролью Hyper-V**  
...
Однозначного ответа на этот вопрос у меня нет. Нужно оценивать конкретные ситуации.  
Создание гетерогенной среды усложняет систему и работу с ней, но может быть оправдано ростом стабильности и скорости работы рабочих нагрузок.
Использование гетерогенных сред виртуализации возможно при чётком понимании преимуществ и недостатков такого решения.

#### Мотивируйте ваш ответ примерами.  

Например, в компании есть развитый отдел продаж и бухгалтерия (особенно при наличии холдинга из нескольких юридических лиц) работающие с 1С и c ЭДО на основе MS SharePoint, есть развитый отдел разработки программного продукта и предполагается активная работа с использованием подключения к виртуальным рабочим столам (требования политики ИБ).    
  
В таком случае возможно такое решение:  
1. Для запуска ПО интересах бухгалтерии отделов продаж (1С и ЭДО) и при наличии достаточно большого количества ВМ c ОС Windows может быть востребована виртуализация на базе Hyper-V.  
2. Для работы разработчиков удобно развернуть среду виртуализации на основе Linux и Docker.  
3. Для реализации удалённой работы с использованием технологии VDI воспользоваться решениями от VMware или Citrix.  
...
Наоборот, при наличии малых количеств ВМ под Windows и отсутствии потребности в VDI (например, при переходе на BYOD при удалённой работе) оптимальным будет переход на гомогенную среду виртуализации (например, KVM или vSphere). При этом более высокие требования по квалификации и ФОТ административного персонала будут компенсированы уменьшением численности сотрудников привлекаемых к поддержке таких систем. А рост CAPEX может быть скомпенсирован возросшим уровнем поддержки среды виртуализации со стороны вендора.    
  
Третий вариант – это слияние/поглощение одной компании другой, при котором для обеспечения бизнеса в разных компаниях изначально использовались разные среды виртуализации. Тут возможны варианты как продолжения поддержки гетерогенной среды, так и консолидация продуктов на основе единых подходов, разрабатываемых после анализа эффективности работы подразделений ИТ.  
  
**Вывод:** Каждый отдельный вариант создания гетерогенных сред должен проходить комплексную оценку перед внедрением. «Исторически сложившиеся» гетерогенные среды как правило такую проверку не проходят и могут быть упрощены до оставления одного-двух продуктов виртуализации в пределах компании.  
